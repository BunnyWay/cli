...
│           │   │       ├── upload.ts     # Upload a local file (<file> positional, --zone, --to, --checksum streams a SHA256, --content-type); prompts for the remote path via uploadDestination when --to is omitted interactively (a blank answer means the zone root; cancelling aborts rather than falling through to it), and a directory argument errors with a hint rather than "File not found"
