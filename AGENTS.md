...
│           │   │   ├── provision.ts       # promptSiteName (normalize/validate, directory-name suggestion) + resolveSiteRegion (uppercase, default DE; pins the ssd tier to DE and rejects a conflicting --region) + createSiteWithProgress (createSite under a step-tracking spinner; shared with create.ts) + createLinkedSite (create + manifest link → SiteContext, skipping create's domain/CI prompts) for the deploy picker's new-site branch
...
│   ├── create          [name] [--region] [--tier hdd|ssd] [--domain] [--link]
│   │                                       Provision a site (idempotent; a failed create re-runs cleanly; each resource is looked up by name first). A missing name comes from `sites.name` in bunny.jsonc (reported in text output), else interactive runs prompt for one (directory-name suggestion). --tier picks the storage zone tier (hdd = Standard, ssd = Edge; create-time only, and ssd forces DE so any other --region is rejected rather than silently rewritten by the API); omitted leaves the API default, and the created zone's tier is reported in both text and JSON output. --domain attaches a custom production domain; when omitted, interactive runs offer to add one (Bunny DNS record with confirmation, nameserver guidance when undelegated, DNS wait + SSL). GitHub repos then get an offer to scaffold the deploy workflow (declining prints it instead).
