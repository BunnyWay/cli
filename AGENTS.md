...
│           │   │   ├── router/source.ts  # routerSource + ROUTER_VERSION (recorded in site state; deploy republishes stale routers via ensureRouterCurrent): the middleware Edge Script, one per site, attached to its pull zone. CRITICAL platform gotcha: at the edge ctx.request.url is the ORIGIN-facing address (http://<edge-ip>:9000/...), NOT the requested host; the client hostname comes from the CDN-Host/Host headers (clientHostname()), and the index-retry probe URL must be rebuilt on that host so it re-enters the CDN instead of hitting unrouted storage paths. Every request → CURRENT_DEPLOY's directory, root-served so client-side routers and root-absolute assets work as-is; /_bunny/* → 403 (the client-sent x-bunny-index-retry header is stripped; the flag is router-internal), trailing-slash → index.html, and a slashless GET/HEAD 404 probes its directory index (re-entrant HEAD of the URL + "/") and 301-redirects to the slash URL when it exists (so /blog resolves with the right relative-URL base, while exact extensionless objects and dotted directories stay reachable). HTML responses (.html path or text/html content type) get `Cache-Control: no-cache, must-revalidate` in onOriginResponse so browsers revalidate every page view and new deploys show up immediately; this requires the pull zone's SITE_CACHE_SETTINGS (both cache overrides on -1, "respect origin"), because any other setting makes the delivery layer rewrite Cache-Control and the CDN cache HTML for its default TTL. The trade: the CDN never stores HTML (each HTML request runs the script and pulls from storage), while assets keep the storage origin's long max-age and cache normally
...
│           │   │   ├── upgrade-router.ts # Republish the site's router script with the CLI's current source, re-apply SITE_CACHE_SETTINGS to the pull zone, and record ROUTER_VERSION in state (deploy also auto-republishes stale routers)
