...
│   │   ├── add         <domain> [site] [--ssl] [--wait] [--no-force-ssl]  Add a domain; also attaches *.preview.<domain> and, only when that wildcard attached, records the domain in site state (onAdded hook; the recorded domain is what switches deploy/CI into preview mode)
