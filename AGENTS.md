...
│   │   │                                   Connection credentials for the zone (one password, shaped per protocol, each with a docs link); prompts for the type when omitted interactively; --format emits a client config (sdk snippet or S3 tool), else table/--output json; table and JSON mask the secret unless --show-secret, then it offers to save the real values to .env
