...
The `.fail()` handler prints the error, a "Did you mean" suggestion when a top-level command or flag is a near miss (`core/suggest.ts`; yargs' own recommendation only fires below the root because of the `$0` default command), and a one-line pointer to `--help` instead of dumping the full help text. Under `--output json` it emits the same `{ error, hint }` payload command errors use.
...
`defineCommand()` also normalizes every 401 to one message with a hint naming the credential source (flag, env, or profile) and the matching `bunny login` command, and rejects `NaN` on any `type: "number"` option or positional before the handler runs, so a typo like `--ttl abc` never reaches the API.
...
- **Site functions are not routed through the site's pull zone.** Each is a standalone Edge Script on its own hostname; the URL reaches the frontend as a build variable (`BUNNY_FUNCTION_<NAME>_URL`, plus a copy with the framework's public prefix) and the bundled entry answers CORS preflight. Same-origin routing with one Change Origin URL rule per function was tried and removed for now: origin-changing rules short-circuit on the first match by `OrderIndex`, indexes must be unique per zone, and re-indexing every rule on each function add or remove made converge fragile. Function scripts are created before the site build (the URLs feed it) and their code is published only after the deploy is validated, so a cancelled or failed deploy never leaves new function code live.
