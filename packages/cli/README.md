...
| `s3`         | `S3_BUCKET`, `AWS_REGION` (or `AWS_DEFAULT_REGION`); optional `S3_PREFIX`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_SESSION_TOKEN`, `S3_PRESIGNED_URL_TTL`, `S3_ENDPOINT` (or `AWS_ENDPOINT_URL_S3`) for S3-compatible providers |
