...
**Importing an existing zone.** `sites create <name> --from-zone <zone>` turns a storage zone you already serve through a pull zone (for example one deployed by a community storage action) into a site, so custom hostnames, certificates, and DNS stay as they are. The import only writes the site state and the two edge rules that block `/_bunny/` and `/deploys/`; what visitors see doesn't change until the first `sites deploy`, which adds the rewrite rule and the sites cache settings and goes live on the existing hostnames. Files already at the zone root stay in storage but stop being served after that deploy; delete them yourself once you're happy. The import refuses a zone that already has a root `deploys` or `_bunny` entry, a pull zone running an edge script, or no storage-backed pull zone; pass `--pull-zone <id>` when several serve the zone. Other edge rules on the pull zone are left in place. `sites delete` treats an imported site like any other and deletes its zones.
...
| `--from-zone`, `--pull-zone`           | `create`                                                                         | Import an existing storage zone (name or ID) instead of creating one; the pull zone to own when several serve it        |
