...
# Bundle a TypeScript entry and its dependencies, then deploy
...
Edge Scripts run on a Deno-based runtime, so `--bundle` follows its rules rather than Bun's: local files and `package.json` dependencies are inlined, `node:` and `npm:` imports are left for the runtime, bare Node builtins (`fs`, `crypto`) are rewritten to `node:` imports, and `jsr:`, URL, and `bun:` imports are rejected. A bundle that references the `Bun` global deploys with a warning, since it fails on the edge unless guarded. Code over the 10 MB Edge Script limit is rejected before upload, bundled or not.
...
bunny sites deploy                                    # a `functions/` directory deploys too, each entry as an Edge Script on its own URL
...
**Functions.** A `functions/` directory next to `bunny.jsonc` (or `sites.functions.dir`) deploys alongside the site, each entry as its own Edge Script. An entry is `functions/hello.ts`, or `functions/hello/index.ts` with whatever it imports, exporting a default `(request: Request) => Response | Promise<Response>` handler (a `{ fetch }` object also works); the CLI bundles it into a single script with no SDK dependency, following the same edge-runtime import rules as `scripts deploy --bundle` (dependencies from `package.json` or `npm:` imports; no `jsr:`, URL, or Bun imports). Names are lowercase letters, digits, and dashes, one entry per name (a `hello.ts` next to a `hello/` folder is rejected), and short enough that `sites-<site>-<name>` fits in 100 characters. A folder with its own `package.json` build script is built with that instead and its `dist/index.js` (or `.ts`/`.mjs`) uploaded as-is, so an existing Edge Script project drops in unchanged. The functions directory never ships as static files, even when the deploy directory contains it. Each script is named `sites-<site>-<name>` and served from its own linked pull zone, so the frontend calls it cross-origin by URL rather than through the site's domain. Function scripts are created before the site builds and their code is published once the deploy is validated, right before the site uploads; every function's URL is passed to `--build` as `BUNNY_FUNCTION_<NAME>_URL` (dashes become underscores) plus a copy with the detected framework's public prefix, such as `VITE_BUNNY_FUNCTION_HELLO_URL`, so `import.meta.env.VITE_BUNNY_FUNCTION_HELLO_URL` works without configuration; a build that runs outside the CLI reads the same URLs from `--output json` (`functions[]`, including any whose folder is gone, marked `stale`) or `sites show`. Bundled handlers answer CORS preflight and allow any origin; set your own `Access-Control-Allow-Origin` header to narrow it. Each function's pull zone delivers the script's own `Cache-Control`, and responses without one default to `no-cache`, which also keeps the CDN from storing them; set `Cache-Control` yourself to cache at the edge. Folder functions get a `.bunny/script.json`, so `bunny scripts env`, `stats`, and `deployments` work from inside the folder. Unchanged function code skips its upload (`--force` redeploys) and `sites delete` removes them with the site. A function whose folder is gone keeps serving until its script is deleted; the deploy warns about it.
