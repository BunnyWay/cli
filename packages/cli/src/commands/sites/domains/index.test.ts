...
// A minimal core client covering the wildcard-attach calls: addHostname POST, hostname list GET, free-cert GET, force-SSL POST.
function stubClient(opts: { failAddHostname?: boolean; failSsl?: boolean }) {
  return {
    ...
    GET: async (route: string) => {
      ...
      if (route === "/pullzone/{id}") {
        return {
          data: { Hostnames: [{ Value: "*.preview.example.com" }] },
        };
      }
      return { data: undefined };
    },
...
// `state.domain` (the previews-operational signal) keys off this return value, so a failed attach must report false.
test("attachPreviewWildcard reports failure when the hostname can't be added", async () => {
  const attached = await attachPreviewWildcard({
    coreClient: stubClient({ failAddHostname: true }),
    pullZoneId: 1,
    domain: "example.com",
    json: true,
  });
