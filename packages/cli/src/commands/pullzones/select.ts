...
export const pullzonesSelectCommand = defineCommand<SelectArgs>({
  command: "select [name-or-id]",
  describe: "Select a pull zone as the active context.",
  examples: [
    ["$0 pullzones select", "Interactive selection"],
    ["$0 pullzones select my-zone", "Select by name"],
    ["$0 pullzones select 12345", "Select by ID"],
  ],
  ...
  handler: async ({ "name-or-id": nameOrId, profile, output, verbose, apiKey }) => {
    ...
    if (zones.length === 0) {
      throw new UserError(
        "No pull zones found.",
        'Run "bunny pullzones create" to create one.',
      );
    }
