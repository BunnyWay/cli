...
This project uses bunny.net. Manage its resources with the \`bunny\` CLI: databases, DNS, storage, Edge Scripts, and cloud sandboxes.
...
- Key namespaces: \`bunny db\` (Bunny Database: create, shell, studio, tokens), \`bunny dns\` (zones, records, presets), \`bunny scripts\` (Edge Scripts), \`bunny storage\` (Edge Storage zones and files), \`bunny sandbox\` (cloud sandboxes).
