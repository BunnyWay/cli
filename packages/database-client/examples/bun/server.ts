/**
 * A notes API on Bun. Bun loads .env for you, so connect() takes no arguments.
 *
...
// Rejects anything that is not a positive integer before it reaches SQLite.
...
const server = Bun.serve({
  ...
  // One place to keep the server's error message generic, whatever the route threw.
  error(cause) {
    console.error(
