/**
 * A notes API on Hono. The same file runs on Bun, Deno, and Edge Scripting,
 * because the default export is a fetch handler:
 *
 *   bun run examples/hono/server.ts
 *
 * On Node, serve it with @hono/node-server instead of the default export.
 */
...
// One handler keeps every database failure out of the response body.
app.onError((cause, c) => {
  console.error(
