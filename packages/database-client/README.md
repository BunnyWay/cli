...
`connect()` reads `BUNNY_DATABASE_URL` and `BUNNY_DATABASE_AUTH_TOKEN` from the environment, which is what `bunny db create --token --save-env` writes and what Edge Scripting already sets:
