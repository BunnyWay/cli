...
test("repository names are trimmed by scanning, not by a backtracking pattern", async () => {
  const { ctx, paths } = fakeClients({
    [`/v2/${ACCOUNT}/myapp/tags/list`]: { tags: [] },
  });

  await registryTags.invoke(ctx, { repository: "///myapp///" });
  expect(paths).toEqual([`/v2/${ACCOUNT}/myapp/tags/list`]);

  // A pathological input resolves instantly; a `/^\/+|\/+$/` pattern rescans from every slash.
  const start = performance.now();
  expect(
    registryTags.invoke(ctx, { repository: "/".repeat(200_000) }),
  ).rejects.toThrow(/Repository name is empty/);
